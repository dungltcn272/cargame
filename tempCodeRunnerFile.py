...
def draw_hs_menu():
    ...
    hs1 = font1.render('Top 1:  ' + str(top[0]) +"    " + str(topc[0]) ,True,(255,255,255))
    hs2 = font2.render('Top 2:  ' + str(top[1]) +"    " + str(topc[1]) ,True,(255,255,255))
    hs3 = font3.render('Top 3:  ' + str(top[2]) +"    " + str(topc[2]) ,True,(255,255,255))
    hs4 = font4.render('Top 4:  ' + str(top[3]) +"    " + str(topc[3]) ,True,(255,255,255))
    hs5 = font5.render('Top 5:  ' + str(top[4]) +"    " + str(topc[4]) ,True,(255,255,255))
    ...
    for event in pygame.event.get():
        ...
        if event.type == KEYDOWN:
            if event.key == K_LEFT and player.rect.center[0] > lan_trai:
                ...
                player.rect.x += 100
            elif event.key == K_UP and player.rect.center[1] > 100:
                player.rect.y -= 50
            elif event.key == K_DOWN and player.rect.center[1] < height-100:
                player.rect.y += 50
        #__Kiểm tra va chạm
    ...
    if game_state == "start_menu":
        ...
        for vehicle in Vehicle_group:
            ...
            if vehicle.rect.top >= height:
                vehicle.kill()
                if score < 20: score += 1
                elif score >= 20 and score < 60: score+=2
                elif score >= 60 and score < 150: score+=3
                elif score >= 150: score+=5
                if score == 20:
                    speed += 1
                if score == 60: speed += 1.5
                if score == 150: speed += 1.5
                if score == 300: speed += 1
        if len(Coin_group) < 3 :
            add_coin = True
            for coinn in Coin_group:
                if coinn.rect.top < coinn.rect.height * 0.5:
                    add_coin = False
            if add_coin: #back______
                coin_lane = random.choice([lan_trai-50,lan_giua-50,lan_phai-50])
                coinn = Coinn(coin_lane,height/-2)
                Coin_group.add(coinn)
        for coin in Coin_group:
            coin.rect.y += speed
            if coin.rect.top >= height:
                coin.kill()
        #for coinc in Coin_group:
            if pygame.sprite.collide_rect(player,coin):
                coin.kill()
                #__arcade.play_sound(cp)
                Coin += 1
                if score < 20: score += 1
                elif score >= 20 and score < 60: score+=2
                elif score >= 60 and score < 150: score+=3
            ...
            if score > top[4]:
                ...
                for i in range(0,4):
                    ...
                    for j in range(i+1,5):
                        if(top[i] < top[j]):
                            idx = j
                        elif top[i]==top[j] and topc[i] < topc[j]:
                            idx = j
                    swap = top[i]
                    top[i] = top[idx]
                    top[idx] = swap
                    ...
                    topc[i] = topc[idx]
                    topc[idx] = swap
            elif score == top[4] and Coin > topc[4]:
                top[4] = score
                topc[4] = Coin
                idx = -1
                for i in range(0,4):
